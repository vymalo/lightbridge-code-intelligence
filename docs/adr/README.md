...
| [0066](0066-deep-tier-external-knowledge-tools.md) | External-knowledge MCP tools, mediated by the control plane: a dynamically-discovered, config-driven registry of in-cluster MCP servers (`knowledge_tools.mcp_servers`, e.g. brave-search/context7) — no hardcoded per-provider tools, query/name not URL (keeps the Job sandboxed, no new secrets); any tier via the normal `review.<tier>.tools` allowlist (`mcp_tools` sentinel), not deep-only | Accepted |
