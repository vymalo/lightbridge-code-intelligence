...
| [0066](0066-deep-tier-external-knowledge-tools.md) | External knowledge for the deep tier: control-plane-mediated `web_search` + Context7 as an MCP client over in-cluster K8s DNS to already-deployed brave-search/context7 MCP servers (query/lib not URL — keeps the Job sandboxed; no new secrets); deep-only via the allowlist + a server-side tier re-check | Accepted |
